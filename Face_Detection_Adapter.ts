...
//const imageURL = "http://192.168.178.124/uwe.jpg"; // Das Bild, welches erkannt werden soll
//const imageURL = "http://192.168.178.90/snap.jpeg"; // Doorbell
const imageURL = "http://192.168.178.173/snap.jpeg"; // Haustüre
const minimumConfidencePercent = 82; // Mindesterkennungsrate des Bildes in Prozent
const minimumDetectionDurationTimeMS = 350;
...
async function doDetection(detectionResultCallback) {
    return new Promise(iterationCallback => {    
        ...
        request.get({url: imageURL, encoding: null},  async function (err, response, body) {
            if (err) {
                throw err;
            }
            ...
            var filename = baseDirectory + "detection_face_" + runningIndex + ".jpg";
                                ...
                                Jimp.loadFont(Jimp.FONT_SANS_32_WHITE, function (err, fontJimp) {
                                    ...
                                    json_object.predictions.forEach(function (prediction) {
                                        if (prediction.confidence > (minimumConfidencePercent/100)) {
                                            ...
                                            imageJimp.scan(prediction.x_min, prediction.y_max+5, (prediction.x_max-prediction.x_min), 40, makeIteratorThatFillsWithColor(0x00000040));
                                            var userName = prediction.userid;
                                            userName = userName.substring(0, prediction.userid.indexOf("_"));
                                            imageJimp.print(fontJimp, prediction.x_min+7, prediction.y_max+8, (userName + " (" + (prediction.confidence*100).toFixed(0)+"%)"));  
                                    ...
                                    var filenameBoundingBox = baseDirectory + "detection_face_boundingbox" + runningIndex + ".jpg";
